...
function HeroSection() {
  return (
    <section className="relative min-h-[200vh] overflow-hidden">
      {/* 3D Scene — sticky, full viewport, front and center */}
      <div className="sticky top-0 h-screen">
        <div className="absolute inset-0">
          <ClientHeroScene />
        </div>

        {/* Minimal gradient — just enough to read text, drone dominates */}
        <div className="absolute bottom-0 left-0 right-0 h-1/3 bg-gradient-to-t from-[var(--color-bg)] to-transparent" />

        {/* Bottom-anchored text — small, out of the drone's way */}
        <div className="absolute bottom-16 left-0 right-0 z-10 px-6">
          <div className="max-w-7xl mx-auto flex items-end justify-between">
            <div>
              <p className="font-mono text-[10px] uppercase tracking-[0.3em] text-[var(--color-accent-light)] mb-2">
                Open Source Drone Electronics
              </p>
              <h1 className="font-display text-4xl md:text-5xl font-bold tracking-tight leading-[1.05]">
                Open
                <span className="text-[var(--color-gold)]">Drone</span>
              </h1>
            </div>
            <div className="hidden md:flex items-center gap-3">
              <Link
                to="/collections/all"
                className="inline-flex items-center gap-2 px-5 py-2.5 bg-[var(--color-gold)] text-[var(--color-bg)] font-mono text-xs font-bold uppercase tracking-wider rounded hover:bg-[var(--color-gold-hover)] transition-colors"
              >
                Shop
              ...
              <a
                ...
                className="inline-flex items-center gap-2 px-5 py-2.5 border border-[var(--color-border)] text-[var(--color-text-muted)] font-mono text-xs uppercase tracking-wider rounded hover:border-[var(--color-text-muted)] transition-colors"
              >
                Source
              </a>
            </div>
        ...
        {/* Scroll indicator */}
        <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1 text-[var(--color-text-muted)]">
          <span className="font-mono text-[9px] uppercase tracking-widest">Scroll</span>
          <div className="w-px h-6 bg-gradient-to-b from-[var(--color-text-muted)] to-transparent animate-pulse" />
        </div>
      </div>
    </section>
  );
