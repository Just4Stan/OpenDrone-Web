...
# dingbats, ligatures. Every OpenType feature, both Inter axes, hinting, the
# glyph-name (post) table and all name records are kept: dropping post
# changed glyph rasterisation by ~1/255 at 1x, measured, so the subset renders
# pixel-identical to the full font for that character set. Anything outside
# falls back to the system font. Full Inter is 352 KB, this subset ~145 KB.
set -eu
SRC="${1:?source dir}"
...
for f in inter-var jetbrains-mono-Regular jetbrains-mono-Medium jetbrains-mono-Bold; do
  python3 -m fontTools.subset "$SRC/$f.woff2" --unicodes="$RANGES" \
    --layout-features='*' --glyph-names --name-IDs='*' --flavor=woff2 \
